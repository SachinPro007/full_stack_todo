...
export async function GET() {
  ...
  const [[session]] = await db.execute("SELECT * FROM session WHERE id = ?;", [
    sessionIdRes,
  ]);
  ...
  const [todos] = await db.execute(
    "SELECT * FROM todos_data WHERE userID = ?;",
    [session.userID],
  );
...
export async function POST(request) {
  ...
  const [[userSession]] = await db.execute(
  ...
  if (!user) {
    ...
    if (!authUser) {
      return Response.json(
        {
          error: "You are not valid User.... Please create Account first....!",
        },
        { status: 401 },
      );
    }
  ...
  await db.execute(
    "INSERT INTO todos_data (id, text, completed, userID) VALUES (?, ?, ?, ?)",
    [newTodo.id, newTodo.text, newTodo.completed, userSession.userID],
  );
